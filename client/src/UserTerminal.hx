...
//	public var spamLog			: MCField;
	...
	public var unlockedPass	: Hash<Bool>; // utilis� pour la reward "mot de passe trouv�"
...
//	}
	...
	function startSoundAnim(s:Sfx, from:Float, to:Float) {
		// rappel sur un son d�j� en train de fader ?
...
//		spamLog._visible = false;
	...
	public function startAnim(type:AnimType, mc:MovieClip, ?str:String, ?delay=0.0, ?fl_linkToProgress=false) {
		var spd = 0.06;
		for (a in animList)
			if ( a.type==type && a.mc==mc ) // TODO : � v�rifier si pb d'animations...
				endAnim(a);
		var data : Float = null;
...
//				dock.onKeyShortcut(c-48);
	...
	// m�thode appel�e 1x par seconde si le timer est actif
	function onTick() {
		tick++;
		vman.onTick(tick);
		// regen mana
...
//			spam(TD.texts.get("spamLine"));
		...
		// NOTE 1 : attention, log et timer d�plac�s avant cinematics ! pas de bug � signaler ?
		// NOTE 2 : net.update et dock.update se trouvaient juste apr�s Progress.update !
		for (a in apps)
			a.update();

		// flood dans l'�cran de boot
		if ( bootCooldown>0 )
			bootCooldown--;
